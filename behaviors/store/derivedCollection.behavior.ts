import { Collection, Container, DerivedState, collection, container, derived } from "@store/index.js";
import { behavior, effect, example, fact, step } from "best-behavior";
import { expect, is } from "great-expectations";
import { testStoreContext } from "./helpers/testStore";

interface DerivedStateContext {
  stateCollection: Collection<string, DerivedState<number>>
    ...
    .script({
      suppose: [
        fact("there is a container and a collection of derivative state", (context) => {
          const derivedStateCollection = collection((id: string) => derived({
            name: `fun-${id}`,
            query: (get) => {
        ...
        fact("a subscriber subscribes to derived state with id", (context) => {
          context.subscribeTo(context.tokens.stateCollection.at("derived-1"), "derived-sub-1")
          context.subscribeTo(context.tokens.stateCollection.at("derived-1"), "derived-sub-2")
        })
      ],
      perform: [
      ...
      observe: [
        ...
        effect("the stringified token shows the name", (context) => {
          expect(context.tokens.stateCollection.at("derived-1").toString(), is("fun-derived-1"))
        })
      ]
    })
