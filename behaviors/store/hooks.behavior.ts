    ...
    .script({
      ...
      observe: [
        effect("the onRegister hook is called with each container", (context) => {
          const containerNames = context.tokens.registeredContainers.map(container => container.toString())
          expect(containerNames, is(arrayWith([
            equalTo("string-container"),
            equalTo("number-container")
          ])))
        }),
