...
import { container, derived, State, value, Value, valueAt } from "@store/index";
...
const externalItemToken = container<ItemWithValue>({
  initialValue: {
    id: "27", name: value("Cool Person")
  }
})
...
export default behavior("list memory", [
  ...
  example(renderContext<ListExamplesState>())
    .description("external token value is referenced in a list item effect")
    .script({
      suppose: [
        fact("there is a list", (context) => {
          context.setState({
            ...
            return (root) => {
              root.div(el => {
                ...
                el.children.textNode(get => {
                  const name = valueAt(externalItemToken, $ => $.name)
                  return `${get(name)} - ${get(item)} - ${get(index)}`
                })
              })
            }
          }
      ...
      observe: [
        effect("the list is rendered with the value", () => {
          const elements: Array<HTMLElement> = Array.from(document.querySelectorAll("div[data-list-item]"))
          expect(elements.map(el => el.innerText), is([
            "Cool Person - a - 0",
            "Cool Person - b - 1",
    ...
    .andThen({
      perform: [
        ...
        step("update the value", (context) => {
          context.writeTo(valueAt(externalItemToken, $ => $.name), "Mr. Cool")
        })
      ],
      observe: [
        effect("the list view updates", () => {
