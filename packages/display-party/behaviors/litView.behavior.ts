...
const simpleViewBehavior = (context: Context<TestAppController>): ConfigurableExample =>
  example(context)
    .description("Rendering a simple lit view")
    .script({
      suppose: [
        fact("a view with an input and button is provided", async (controller) => {
          await controller.loadLitApp("simpleLitView.app")
        })
      ],
      observe: [
      ...
      perform: [
        step("a new name is input", async (controller) => {
          await controller.display.select("input[data-name-input]").type("Fun Person")
    ...
    .script({
      suppose: [
        fact("some state is provided for the view", async (controller) => {
          await controller.loadLitApp("nestedLitView.app")
        })
      ],
      observe: [
...
export default (context: Context<TestAppController>) => behavior("lit view", [
