...
`@spheres/view` provides a Typescript API for building views with HTML
...
value wraps functions passed to stateful attributes or text nodes. This
allows you to use the arguments when defining attributes or text nodes.
...
views that have a static structure with attributes values or text that depends
