...
// ReactiveVariables -- and Variables in general seem to be too
// dangerous ... we should not expose these as part of the public API
...
export class ReactiveVariable<T> extends State<T> {
  ...
  assignState(token: Container<T>) {
    if (!this.store) {
      // this can happen because on the first run of the reactive effect
      // we set the value of this with a particular token
  ...
  // Need a store test for this
  // assignValue(val: T) {
  //   if (!this.store) return
  //   const controller = this.store[getContainerController](this)
...
export class Store {
  ...
  [setController]<T>(token: State<T>, controller: StateController<T>) {
    // NOTE -- This needs a test since we should use getKeyForToken probably!!!
