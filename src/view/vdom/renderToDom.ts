...
export function createDOMRenderer(store: Store): DOMRenderer {
  return (element, node) => {
    // we don't actually want to use patch anymore
    // const rootNode = patch(store, virtualize(element), node)
    // const rootNode = insert(store, virtualize(element), node)
...
export function createNode(store: Store, vnode: VirtualNode): Node {
  switch (vnode.type) {
    ...
    case NodeType.ZONE_LIST:
      // This won't work since the ListEffect needs to have the parent
      // element. But we don't have that yet ...
      // WE could pass in the parent but I'm not sure that's a good idea
    ...
    default:
      ...
      // We are adding events here ... instead should we delegate these
